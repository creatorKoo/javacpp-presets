...
case $PLATFORM in
    ...
    macosx-x86_64)
        cd libusb-$LIBUSB_VERSION
        ./configure --prefix=$INSTALL_PATH --disable-shared --with-pic --disable-udev
        make -j $MAKEJ
        make install
        cd ../glfw-$GLFW_VERSION
